...
bool Consumer::Init(const std::string& groupid, const std::string& topic) {
  ...
  EventCb event_cb;
  if (conf_->set("event_cb", &event_cb, errstr) != RdKafka::Conf::CONF_OK) {
    spdlog::error("Failed to set event_cb: {}", errstr);
    return false;
  }
  RebalanceCb rebalance_cb;
  if (conf_->set("rebalance_cb", &rebalance_cb, errstr) != RdKafka::Conf::CONF_OK) {
    spdlog::error("Failed to set event_cb: {}", errstr);
    return false;
  }
  // 创建生产者
